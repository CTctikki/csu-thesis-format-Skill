...
- 目录和页码可以用 Word 自动生成。
- 真实定稿时，若已经人工把 `TOC1/TOC2` 调整为单页且视觉正确，优先保留该工作样式，不要仅因为模板默认样式残留而强行刷回。
- 当前验证过的可交稿形态中，目录条目通常使用小四号（约 12 pt）；其中 `TOC2` 保留两个汉字左右缩进。
- 全文只采用一种编号体系，不要混用。
- 格式一，理工科常用：
...
- 每一章单独起页。
- 正文文献引用：使用方括号并以上标形式标注，例如 `[1]`、`[1-3]`、`[14,15]`；参考文献列表条目前的 `[1]`、`[2]` 保持普通基线文本。
- 正文和图题、表题、表格内的特征符号应统一为真正的上下标格式，不要只用普通字符近似显示。常见材料机器学习特征写法包括：`d_WC`、`f_WC`、`x_Ni`、`x_Co`、`a_WC`、`A_sp`、`T_norm`；派生量或单位中的指数应使用上标，如 `f_WC^(2/3)`、`R^2`、`cm^-3`、`μm^-1`、`mm^2/s`。
...
- 在真实 DOCX 中，这一要求常通过专门的 `书目1` 样式实现：`楷体_GB2312 + Times New Roman + 10.5 pt + 悬挂缩进`。检查时优先看样式链，不要只看第一段 run 的直接格式。
